...
test.before(async()=>{
  ...
  await exec('PRAGMA foreign_keys=ON; CREATE TABLE companies(id INTEGER PRIMARY KEY,name TEXT NOT NULL); CREATE TABLE users(id INTEGER PRIMARY KEY); CREATE TABLE tickets(id INTEGER PRIMARY KEY); INSERT INTO companies VALUES(1,\'A\'),(2,\'B\'); INSERT INTO users VALUES(1); '+migration.up);
...
test('CRUD de responsáveis respeita e-mail, empresa, unicidade ativa e inativação',async()=>{
  const invalid=await request('POST','/api/visits/managers',{company_id:1,contact_type:'primary_manager',name:'Gestor',email:'invalido',job_title:'Diretor'});
  assert.equal(invalid.status,400);
  const first=await request('POST','/api/visits/managers',{company_id:1,contact_type:'primary_manager',name:'Gestor',email:'gestor@example.com',job_title:'Diretor'});
  assert.equal(first.status,201);
  const duplicate=await request('POST','/api/visits/managers',{company_id:1,contact_type:'primary_manager',name:'Outro',email:'outro@example.com',job_title:'Diretor'});
  assert.equal(duplicate.status,409);
  assert.equal((await request('PUT','/api/visits/managers/'+first.body.id,{active:0})).body.active,0);
  assert.equal((await request('POST','/api/visits/managers',{company_id:1,contact_type:'primary_manager',name:'Novo',email:'novo@example.com',job_title:'Diretor'})).status,201);
  const substitute=await request('POST','/api/visits/managers',{company_id:1,contact_type:'substitute',name:'Sub',email:'sub@example.com',job_title:'Coordenação'});
  assert.equal(substitute.status,201);
  assert.equal((await request('POST','/api/visits/managers',{company_id:1,contact_type:'substitute',name:'Sub2',email:'sub2@example.com',job_title:'Coordenação'})).status,409);
  assert.equal((await request('PUT','/api/visits/managers/'+substitute.body.id,{company_id:2})).status,400);
  const list=await request('GET','/api/visits/managers?companyId=1');
...
test('CRUD de setores aceita unidade opcional e rejeita unidade de outra empresa',async()=>{
  ...
  assert.equal(updated.status,200);assert.equal(updated.body.active,0);
  assert.equal((await request('GET','/api/visits/departments?companyId=1')).body.length,2);
